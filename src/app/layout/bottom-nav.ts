...
import { lucideLayers, lucideNewspaper, lucideSearch, lucideUser } from '@ng-icons/lucide';
import { NAVIGATION } from '@/shared/config';

/**
 * 좁은 화면의 메뉴입니다. 상단 바는 로고와 테마 전환만 남기고 이동 수단을 여기로 옮깁니다.
 *
 * 검색이 네 번째 칸입니다. 띄우는 버튼(FAB)을 쓰지 않는 이유는 두 가지입니다. 스크롤 중에
...
@Component({
  ...
  providers: [provideIcons({ lucideNewspaper, lucideLayers, lucideUser, lucideSearch })],
  ...
  template: `
    ...
    <nav aria-label="주요 메뉴" class="grid grid-cols-4 pb-[env(safe-area-inset-bottom)]">
      @for (item of navigation; track item.path) {
        <a
          [routerLink]="item.path"
          [routerLinkActiveOptions]="{ exact: item.exact }"
          routerLinkActive="text-primary"
          class="flex h-14 flex-col items-center justify-center gap-0.5 text-[0.6875rem] text-foreground-secondary transition-colors"
        >
