 ...
 *
 * 골격 종류와 스크롤 컨테이너의 대응은 external/refarch-angular-springboot/docs/architecture/angular/references/layout.md 2절이 원본입니다.
 ...
 * 없으며(adaptive-ui.md 6절), CSS 로 해결되는 것을 런타임 분기로 만드는 것도 1절이 금지합니다.
 */
@Component({
  ...
  template: `
    ...
    <div class="mx-auto flex max-w-[90rem]">
      ...
      <!--
        flex 자식의 기본 min-width 는 auto 라 긴 표와 코드 블록이 열을 밀어냅니다. layout.md 4.2절

        베일의 기준 상자입니다. 이 프레임은 사이드바를 함께 그리므로 셸의 본문 영역을
        덮으면 사이드바까지 가려집니다. 문서 영역에서 이동 수단이 사이드바이므로
        대기 중에도 눌러야 합니다(loading.md 7.1절).
      -->
    ...
    <!--
      ...
      번들 문제를 측정으로 확인한 뒤에만 지연을 적용한다는 규칙은 adaptive-ui.md 4.4절입니다.
    ...
    @defer (when mobileNavOpen(); prefetch on idle) {
      <!--
        ...
        다른 문서로 이동해도 이전 스크롤 위치에 남습니다. layout.md 4.4절 참조.
      -->
      <hlm-sheet
        side="left"
    ...
    <!-- 데스크탑 사이드바와 모바일 시트가 같은 목록을 공유합니다. layout.md 3절 -->
...
export class DocsLayout {
   ...
   *
   * 주소에 두지 않는 이유는 routing.md 3.3절이 열린 아코디언을 컴포넌트 상태로
  ...
  protected readonly tree = computed<readonly NavigationNode[]>(() => {
    // 템플릿에서 함수를 부르지 않도록 펼침 여부까지 여기서 계산해 둡니다. performance.md 3.1절
