...
[보안](../references/보안.md) §3.1 은 `bypassSecurityTrust*` 호출을 금지하며, 서식 있는 텍스트는 컴포넌트로 렌더링하고 HTML 문자열을 주입하지 말라고 규정합니다. 그 금지의 근거는 **서버가 신뢰 가능해도 서버가 받은 값은 사용자 입력**이라는 점입니다.
...
| 구분 | 내용 |
| :--- | :--- |
| **장점** | HTML 문자열 주입이 사라져 §3.1 을 그대로 지킵니다. 블록별로 컴포넌트를 교체할 수 있습니다 |
