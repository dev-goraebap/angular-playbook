...
- **날짜 선택기를 직접 만들어야 합니다.** 네이티브 위임을 기각한 직접적 결과입니다. 날짜 선택기는 접근성 결함이 가장 자주 나오는 컴포넌트이므로, brain이 제공하는 프리미티브의 키보드 조작과 ARIA 지원 범위가 중요해집니다. 이 범위는 미확인 상태입니다.
...
`@spartan-ng/brain`이 CDK Overlay의 위치 전략 교체를 외부에 허용하는지 미확인입니다. 허용하지 않으면 패턴 1을 사용할 수 없고 모든 적응형 구현이 패턴 2로 일원화됩니다. 이 경우 본 ADR의 판정 순서를 개정합니다.
