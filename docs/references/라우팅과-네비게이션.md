...
경로 세그먼트는 **ASCII 로 작성합니다.** 한글을 포함한 경로는 정적 생성까지는 통과하지만 서빙 단계에서 404가 됩니다. 빌드가 성공하고 산출물도 존재하므로 실제로 요청해 보기 전에는 드러나지 않습니다. 실측 근거는 [개발 환경](개발-환경.md) §7에 있습니다.
...
`pages` 슬라이스의 `index.ts`는 라우트 진입 컴포넌트만 내보냅니다. 근거는 [패키지 배치와 참조 규칙](패키지-배치와-참조-규칙.md) §7.4에 있습니다.
