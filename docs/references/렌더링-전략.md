...
| 항목 | 확인할 내용 | 영향 |
| :--- | :--- | :--- |
| **미분류 파일 경고** | Steiger가 `src/main.ts`·`server.ts`를 계층 밖 파일로 경고하는지 | 경고 시 설정에서 제외 |
| **배럴과 청크 분리** | `loadComponent`가 슬라이스 `index.ts`를 경유할 때 청크가 분리되는지 | 깨지면 배럴 범위를 좁히거나 예외를 등재 |
