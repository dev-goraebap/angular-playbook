...
변환 계층을 두면 백엔드 변경이 매핑 함수에서 멈추고 화면 어휘를 독립적으로 유지할 수 있습니다. 그럼에도 두지 않는 이유는 **백엔드가 필드를 자주 추가한다는 전제**가 [아키텍처 2절](../아키텍처.md)에 기재되어 있기 때문입니다. 필드 하나가 추가될 때마다 매핑을 고쳐야 하며, 그 작업은 값을 옮겨 담기만 할 뿐 판단을 담지 않습니다.
