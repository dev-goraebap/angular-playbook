...
slug: architecture
title: '프론트엔드 아키텍처'
description: 'Angular 프론트엔드 애플리케이션의 구조와 설계 결정 명세'
order: 0
---
...
각 항목의 세부 규칙은 `references/` 디렉터리의 개별 참조 문서가 원본입니다. 본 문서는 목차와 판단 근거만 제공합니다.

| 찾고자 하는 항목 | 관련 섹션 및 참조 문서 |
| :--- | :--- |
| **새 코드를 어디에 둘 것인가** | 8.1절 → [패키지 배치와 참조 규칙](references/패키지-배치와-참조-규칙.md) |
| **서버 데이터의 조회와 캐시** | 8.2절 → [서버 상태와 클라이언트 상태](references/서버-상태와-클라이언트-상태.md) |
| **대기 중 무엇을 보여주는가** | 8.3절 → [로딩 전략](references/로딩-전략.md) |
| **SSR · 하이드레이션 경계** | 8.4절 → [렌더링 전략](references/렌더링-전략.md) |
| **서버 타입과 화면 모델** | 8.5절 → [API 계약 소비](references/API-계약-소비.md) |
| **화면 골격과 스크롤** | 8.6절 → [레이아웃](references/레이아웃.md) |
| **화면 이동과 URL 상태** | 8.7절 → [라우팅과 네비게이션](references/라우팅과-네비게이션.md) |
| **폼 검증의 역할 분담** | 8.8절 → [폼과 검증](references/폼과-검증.md) |
| **컴포넌트의 상태 소유** | 8.9절 → [컴포넌트 설계](references/컴포넌트-설계.md) |
| **UI 킷과 디자인 토큰** | 8.10절 → [디자인 시스템과 토큰](references/디자인-시스템과-토큰.md) |
| **기기별 UI 전환** | 8.11절 → [적응형 UI](references/적응형-UI.md) |
| **토큰 보관과 XSS 대응** | 8.12절 → [보안](references/보안.md) |
| **에러 표시와 관측** | 8.13절 → [예외 · 에러 표시 · 로깅](references/예외-에러표시-로깅.md) |
| **키보드 조작과 스크린리더** | 8.14절 → [접근성](references/접근성.md) |
| **번들 예산과 측정** | 8.15절 → [성능](references/성능.md) |
| **파일명과 식별자 규약** | 8.16절 → [명명 규칙](references/명명-규칙.md) |
| **검증 수준과 도구** | 8.17절 → [테스트](references/테스트.md) |
| **구성 파일과 실행 명령** | 2절 → [개발 환경](references/개발-환경.md) |
| **아키텍처 결정 기록 (ADR)** | 9절 → `decisions/` |
...
주된 독자는 **작성자 본인과 AI 코딩 에이전트**입니다. 에이전트가 규칙을 읽고 코드를 올바른 위치에 배치하는 것이 1차 목적이며, 사람이 읽는 것은 2차입니다. 이 우선순위가 1절의 품질 목표와 4절의 전략 선택을 결정합니다.
...
성능과 접근성은 품질 목표 서열에 포함하지 않습니다. 두 항목 모두 10절의 검증 항목과 11절의 자동 강제 수단으로는 등재하되, 목표 서열에서 제외한 이유는 프로젝트마다 요구 수준이 크게 달라 규범집 차원의 순위를 정할 수 없기 때문입니다.

상기 세 목표의 우선순위는 4절의 아키텍처 전략을 결정하는 기준입니다. FSD를 채택하고 `widgets` 계층을 도입하지 않은 근거는 **예측 가능성(목표 1)** 이며, 조회 수단으로 외부 라이브러리 대신 프레임워크 내장 API를 우선한 근거도 같습니다.
...
시스템 구조 선택 및 아키텍처 결정(4절)의 전제가 되는 제약사항입니다.
...
본 문서는 규범집이므로 특정 시스템의 외부 연계나 배포 환경을 알 수 없습니다. arc42 3절과 7절은 이 제약의 직접적 결과이며, 해당 절에서 구체 구성 대신 판단 기준만 제시하거나 프로젝트 문서로 위임합니다.

### 백엔드 분리

- 백엔드는 별도 저장소로 관리되며 REST API 계약이 유일한 인터페이스입니다.
- 계약 변경 시 양측 사전 협의가 필요하며, 기존 응답 필드 삭제보다 필드 추가의 변경 비용이 낮습니다.
- 이 비대칭은 8.5절에서 생성 타입을 상위 계층에 그대로 노출하기로 한 결정의 근거입니다.
...
`httpResource`는 교차 화면 캐시, 요청 중복 제거, 키 단위 무효화를 제공하지 않습니다. 이 기능들이 실측으로 필요해지면 TanStack Query 도입을 ADR로 판단합니다. 도입 이전까지는 무효화 신호를 `shared`에 두어 계층 위반 없이 해소합니다(8.2절).

목록 필터나 페이지 번호를 컴포넌트 상태가 아닌 URL에 두는 이유는 두 가지입니다. 새로고침·뒤로가기·링크 공유가 별도 구현 없이 동작하고, **로딩 표현의 판정이 URL 비교로 자동 결정되기 때문**입니다(8.3절).
...
이 결정은 8.11절의 적응형 UI 제약과 직접 연결됩니다. 서버에는 포인터와 뷰포트가 없어 적응형 컴포넌트가 하이드레이션 불일치를 일으키는데, 업무 화면 전체가 `Client` 모드이므로 이 위험에서 벗어납니다.
...
계층을 넘나드는 공통 호출 규칙은 8.1절과 8.2절에 있으며, 조회·명령·무효화의 흐름은 [서버 상태와 클라이언트 상태](references/서버-상태와-클라이언트-상태.md)가 원본입니다.

## 7. 배포 뷰 (Deployment View)

**해당 없음.** 본 문서는 규범집이므로 배포 대상 환경과 파이프라인을 알 수 없습니다.

각 프로젝트가 자신의 `docs/아키텍처.md` 7절에서 실제 브랜치 전략과 배포 파이프라인을 기술합니다. 파이프라인 정의의 원본은 각 프로젝트의 CI 설정 파일입니다.
...
주요 설계 선택의 사유와 기각한 대안은 `decisions/` 디렉터리의 ADR로 관리합니다.
...
1절에서 수립한 품질 목표를 검증 가능한 시나리오로 정의합니다.
